...
class WebhookViettelpostController(Controller):
    ...
    @staticmethod
    def _validate_payload_webhook_giaohangnhanh(payload: Dict[str, Any]):
        if not payload:
            return invalid_response(f'The payload is required.', STATUS_BAD_REQUEST)
        elif 'OrderCode' not in payload:
            return invalid_response(f'The field OrderCode missing.', STATUS_BAD_REQUEST)
        elif 'Description' not in payload:
            return invalid_response(f'The field Description missing.', STATUS_BAD_REQUEST)
        elif not payload.get('OrderCode'):
            return invalid_response(f'The value of field OrderCode is empty.', STATUS_BAD_REQUEST)
        elif not payload.get('Description'):
            return invalid_response(f'The value of field Description is empty.', STATUS_BAD_REQUEST)
        else:
            return True
