...
class CreateStoreWizard(models.Model):
    ...
    wid = fields.Many2one('ghn.ward', string='Ward', required=True)
    did = fields.Many2one('ghn.district', string='District', required=True)

    @api.model
    def create_store(self):
        action = self.env.ref('ghn_connector.create_store_wizard_action').read()[0]
        return action
